...
def to_mod_list(employee_list):
   ...
   converse= map(mod, employee_list)
   mod_emp_list=[]
   for x in converse:
...
def map_id_to_initial(employee_list):
   ...
   for x in employee_list:
      new_dict={key:x.get(key) for key in x}
      i=0
      new_diction={}  
      for x in str(new_dict["name"]):
         if i==0:
            new_diction[x]=new_dict["id"]
            i=i+1
      dict.update(new_diction)
   return dict
